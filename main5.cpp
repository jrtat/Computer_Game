...
// 扩展时的参数
const int counter = 3; // 在被模拟多少次后扩展
...
public:
	...
	TreeNode* Expand() { // 错的 UntriedMove
		...
		/* 向搜素树添加节点 */
		for (auto iter = UntriedMoves.begin(); iter != UntriedMoves.end(); iter++) { // 错的
...
void Calc_Potential() { // 计算双威胁值 用到CurBoard
	...
	*/
	...
	while (!Redpl.empty()) {
		if (Redpl.top().pot == val1 && Redpl.top().act >= val3 - 1) {
			HighVal.push_back(Coord(Redpl.top().x - 1, Redpl.top().y - 1)); //记得恢复原来的坐标(0-10)
			Redpl.pop();
		}
		else break;
	...
	while (!Bluepl.empty()) {
		if (Bluepl.top().pot == val2 && Bluepl.top().act >= val4 - 1) {
...
void MCTS(int lstX, int lstY) {
	...
	while (1) {
		...
		if (chrono::duration_cast<chrono::milliseconds>(stops - start).count() >= 29999) { // 提前1ms结束
			return;
		}
