...
public:
	...
	TreeNode* Expand() {
		...
		for (auto iter = UntriedMoves.begin(); iter != UntriedMoves.end(); iter++) {
			...
			int mine = 0, yours = 0, init_n = 0, init_val = 0;

			for (int i = 0; i < Choose.size(); i++) {
				if (iter->x == Choose[i].x && iter->y == Choose[i].y) {
					init_n = 10, init_val = 8;
				}
			}
			for (int i = 0; i < 6; i++) {
	...
	double Simulate() {
		...
		while (1) {
			...
			for (auto iter = UntriedMoves.begin(); iter != UntriedMoves.end();) { // 删除该点
				if (curBoard[iter->x][iter->y] != 0) { // 恢复二者之间的一致对应
					// cout <<" ? " << iter->x << " " << iter->y << endl;
					iter = UntriedMoves.erase(iter);
					if (iter == UntriedMoves.end())break;
				}
				if (iter->x == cx && iter->y == cy) {
					iter = UntriedMoves.erase(iter);
				}
				else iter++;
			}
			...
			// 更新 lastX， lastY
			lastX = cx, lastY = cy;
			// cout << -lastplayer<<" go:" << lastX << " " << lastY <<" Remain:"<< UntriedMoves.size()<< endl;
...
void MCTS(int lstX, int lstY) {
	...
	while (1) {
		...
		*/
		...
		if (chrono::duration_cast<chrono::milliseconds>(stops - start).count() >= 800) { // 提前1ms结束
			break;
		}
